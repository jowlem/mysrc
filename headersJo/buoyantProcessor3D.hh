...
#ifndef BUOYANT_THERMAL_PROCESSOR_3D_HH
#define BUOYANT_THERMAL_PROCESSOR_3D_HH

#include "buoyantProcessor3D.h"
...
template< typename T,
        ...
        >
BuoyanTermProcessor3D<T,FluidDescriptor,TemperatureDescriptor>::
        BuoyanTermProcessor3D(T gravity_, T rho0_, T rhoP_, T Vs_, Array<T,FluidDescriptor<T>::d> dir_)
    :  gravity(gravity_), rho0(rho0_), rhoP(rhoP_), Vs(Vs_),
       dir(dir_)
{
...
template< typename T,
          ...
          template<typename U2> class TemperatureDescriptor
        >
void BuoyanTermProcessor3D<T,FluidDescriptor,TemperatureDescriptor>::process (
        Box3D domain,
        ...
        BlockLattice3D<T,TemperatureDescriptor>& temperature )
{
    ...
    for (plint iX=domain.x0; iX<=domain.x1; ++iX)
    {
		...
		if(localVolfrac != 0) vel += v_sed;
                ...
                {
			force[iD] = -localVolfrac * gravOverrho0[iD] * diffT * 0.02;
		//	force[iD] = 0.;
...
template< typename T,
        ...
        >
BuoyanTermProcessor3D<T,FluidDescriptor,TemperatureDescriptor>*
    BuoyanTermProcessor3D<T,FluidDescriptor,TemperatureDescriptor>::clone() const
{
    return new BuoyanTermProcessor3D<T,FluidDescriptor,TemperatureDescriptor>(*this);
}
